...
public class ProceduralGeneration : MonoBehaviour
{
    ...
    void Generation()
    {
        for (int x = -9; x < width; x++)//This will help spawn a tile on the x axis
        {
            // now for procedural generation we need to gradually increase and decrease the height value
            int minHeight = height - 1;
            int maxHeight = height + 2;
            height = Random.Range(minHeight, maxHeight);
            int minStoneSpawnDistance = height/4 - minStoneheight;
            int maxStoneSpawnDistance = height/4 - maxStoneHeight;
            int totalStoneSpawnDistance = Random.Range(minStoneSpawnDistance, maxStoneSpawnDistance);
            //Perlin noise.
            for (int y = -5; y < height; y++)//This will help spawn a tile on the y axis
            {
